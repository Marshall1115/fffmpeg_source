...
=item B<-thread_message_queue> I<size> B<(>I<input>B<)>
...
Set the contrast value. It accepts a float value in range C<-2.0> to
C<2.0>. The default value is C<0.0>.


=item B<brightness>

Set the brightness value. It accepts a float value in range C<-1.0> to
C<1.0>. The default value is C<0.0>.


=item B<saturation>

Set the saturation value. It accepts a float value in range C<0.0> to
C<3.0>. The default value is C<1.0>.


=item B<gamma>

Set the gamma value. It accepts a float value in range C<0.1> to C<10.0>.
...
C<0.1> to C<10.0>. The default value is C<1.0>.
...
float value in range C<0.0> to C<1.0>.A value of C<0.0> turns the
gamma correction all the way down while C<1.0> leaves it at its full strength.
Default is C<1.0>.
