                    								...
                    								//随后赋值octx->global_opts 对应的值
{
	...
	/*将命令参数和对应的值保存在结构体OptionGroup的成员Option
	 *nb_opts+1
	 */
...
//OptionParseContext中的group_def 指针指向groups
...
void unsplit_commandline(OptionParseContext *octx)
{
    int i, j;
                      ...
                      const OptionGroupDef *groups, int nb_groups)// //nb_groups = 2
{
    ...
    while (optindex < argc) {
        ...
        if (opt[0] == '-' && opt[1] == '-' && !opt[2]) {//-- 无值 错误的命令继续下一次循环
            dashdash = optindex;
            continue;
        }
...
do {                                                                           \
    ...
    if (!arg) {                                                                \
        ...
        if ((ret = match_group_separator(groups, nb_groups, opt)) >= 0) {
            GET_ARG(arg);
		   /*当一组命令分配好后
            *当前组命令写进OptionGroupList
            *置零cur_group数据，重新解析其他组
            */
